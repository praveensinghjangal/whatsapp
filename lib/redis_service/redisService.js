...
class RedisService {
    ...
    getWabaDataByPhoneNumber(wabaNumber) {
        __logger.info('redisService: getWabaDataByPhoneNumber(' + wabaNumber + ')', wabaNumber)
        ...
        __db.redis.get(wabaNumber)
            ...
            .catch(err => {
                __logger.error('redisService: getWabaDataByPhoneNumber(' + wabaNumber + '):', err)
    ...
    setDataInRedis(wabaNumber) {
        ...
        wabaService.getWabaDataFromDb(wabaNumber)
            .then(data => {
                __logger.info('redisService: setDataInRedis(' + wabaNumber + '): setting new redis data', JSON.stringify(data));
                dataObj = data
            ...
            .catch(err => {
                __logger.error('redisService: SetDatainRedis(' + wabaNumber + '):', err)
        ...
        __db.mysql.query(__constants.HW_MYSQL_NAME, queryProvider.setTemplatesInRedisForWabaPhoneNumber(), [wabaPhoneNumber])
            .then(result => {
                if (result && result.length === 0) {
                    __logger.error('redisService: setTemplatesInRedisForWabaPhoneNumber(' + wabaPhoneNumber + '): No Records Found:')
    ...
    setFacebookAuthKeysInRedis(tokenData, wabaNumber, serviceProviderId, userId) {
        __logger.warn('redisService: setFacebookAuthKeysInRedis(' + wabaNumber + '): ', tokenData)
    ...
    getFacebookAuthKeys(wabaNumber) {
        ...
        __db.redis.get(`${token}${wabaNumber}`)
            ...
            .then(data => dataFetched.resolve(data))
            .catch(err => {
                __logger.error('redisService: getFacebookAuthKeys(' + wabaNumber + '): redis error:', err)
                dataFetched.reject({ type: err.type || __constants.RESPONSE_MESSAGES.SERVER_ERROR, err: err.err || err })
            })
        return dataFetched.promise
    }
    ...
    setOptinTemplateInRedis(redisKey, authToken) {
        ...
        __logger.info('Inside function to set the optin template in redis', { redisKey })
        let statusData = {}
    ...
    getOptinTemplateId(wabaPhoneNumber, authToken) {
        ...
        __logger.info('Inside function to get the optin template id', { wabaPhoneNumber })
    ...
    getFbTemplateName(templateid) {
        __logger.info('inside getWabaDataByPhoneNumber', templateid)
        const dataFetched = q.defer()
    ...
    setFbTemplateName(templateid) {
        __logger.info('setFbTemplateName set data in redis called:', templateid)
    ...
    setCountOfRetry(wabaNumber, audienceNumber) {
        const dataInRedis = q.defer()
        const namespace = __constants.RETRY_FOR_J_AND_J
        console.log('[Started', namespace)
        __db.redis.l_push(`${namespace}:${wabaNumber}__${audienceNumber}`, 'a')
            .then(data => dataInRedis.resolve(true))
            .catch(err => {
                __logger.error('redisService: setCountOfRetry(' + wabaNumber + '): catch:', err)
                dataInRedis.reject({ type: err.type || __constants.RESPONSE_MESSAGES.SERVER_ERROR, err: err.err || err })
            })
        return dataInRedis.promise
    }
