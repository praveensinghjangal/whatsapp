...
class RedisService {
  ...
  getWabaDataByPhoneNumber (wabaNumber) {
    ...
    __db.redis.get(wabaNumber)
      .then(data => {
        // console.log('dataatatatatwaba ==> ', data, typeof data)
        if (data) {
  ...
  getWabaNumberByUserId (userId) {
    __logger.info('inside getWabaNumberByUserId', userId)
    const dataFetched = q.defer()
    __db.redis.get('wnbui-' + userId)
      .then(data => {
        // console.log('dataatatatatwaba ==> ', data, typeof data)
        if (data) {
          data = JSON.parse(data)
          ...
          return this.setWabaNumberByUserId(userId)
        }
      })
      .then(data => dataFetched.resolve(data))
      .catch(err => {
  ...
  setWabaNumberByUserId (userId) {
    __logger.info('set data in redis called:', userId)
    const dataUploaded = q.defer()
    const url = __config.wabaPlatformUrl + __constants.INTERNAL_END_POINTS.getWabaNumberByUserId + '?userId=' + userId
    const options = {
      url,
      body: {},
    ...
    request.get(options, (err, httpResponse, body) => {
      __logger.info('getWabaNumberByUserId API response :', body)
      if (body.msg === __constants.RESPONSE_MESSAGES.NOT_AUTHORIZED.message) {
        dataUploaded.reject({ type: __constants.RESPONSE_MESSAGES.SERVER_ERROR, err: { } })
      } else if (err) {
        dataUploaded.reject({ type: __constants.RESPONSE_MESSAGES.SERVER_ERROR, err: { } })
      } else if (body && body.code === 2000) {
        const dataObj = { id: 'wnbui-' + userId, wabaPhoneNumber: body.data.wabaPhoneNumber }
        __db.redis.setex(dataObj.id, JSON.stringify(dataObj), __constants.REDIS_TTL.wabaData)
          .then(data => dataUploaded.resolve(dataObj))
          .catch(err => {
            dataUploaded.reject({ type: err.type || __constants.RESPONSE_MESSAGES.SERVER_ERROR, err: err.err || err })
          })
      } else {
        dataUploaded.reject({ type: __constants.RESPONSE_MESSAGES.NO_RECORDS_FOUND, err: { } })
      }
    })
    return dataUploaded.promise
  }
