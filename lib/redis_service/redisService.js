...
class RedisService {
  ...
  setFacebookAuthKeysInRedis (tokenData, wabaNumber, serviceProviderId) {
    ...
    __db.redis.setex(`token:${wabaNumber}`, JSON.stringify(tokenData), Math.ceil(tokenData.timeLeftToExpire / 1000)) // divide by 1000 to convert ms to second
      .then(data => __db.redis.setex(`token_expiry_identification_key:${wabaNumber}__${serviceProviderId}`, '1', Math.ceil((tokenData.timeLeftToExpire - __constants.FB_REDIS_KEY_BUFFER_TIME + 1000) / 1000))) // adding 1 second so that when this key expires the calling func in fb integration layer always gets condition of < buffer as true)
