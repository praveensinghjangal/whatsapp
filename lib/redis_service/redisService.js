...
class RedisService {
  ...
  setFacebookKeyInRedis (key1, value, exp, key2) {
    const dataInRedis = q.defer()
    var expiry = moment(exp)
    var duration = moment.duration(expiry)
    const ttl1 = duration.asMinutes()
    console.log('ttttlllll', ttl1)
    const ttl2 = ttl1 - 30

    __db.redis.setex(`token:${key1}`, JSON.stringify(value), ttl1)
      .then((data) => {
        return __db.redis.setex(`token:${key2}`, JSON.stringify(1), ttl2)
      }).then((data) => {
        return dataInRedis.resolve(data)
      })
      .catch(err => {
        console.log(err)
