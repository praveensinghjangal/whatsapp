...
class RedisService {
    ...
    setFacebookAuthKeysInRedis(tokenData, wabaNumber, serviceProviderId, userId) {
        console.log('====setFacebookAuthKeysInRedis===', tokenData)
    ...
    getOptinTemplateFromRedis(redisKey) {
        ...
        __logger.info('Inside function to get the optin template from redis', { redisKey })
    ...
    setStaticTemplateForInternalUse(wabaNumber, audienceNumber, messageId) {
        ...
        console.log('namespace, wabanumber, audience number , messageId', namespace, wabaNumber, audienceNumber, messageId)
        __db.redis.setex(`${namespace}:${wabaNumber}__${audienceNumber}`, JSON.stringify(messageId), __constants.FB_REDIS_KEY_STATIC_TEMPLATE_SET_TIME)
            ...
            .catch(err => {
                console.log('error in setStaticTemplateForInternalUse', err)
