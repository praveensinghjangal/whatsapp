module.exports = {
  ...
  templateStatusUpdate: (statusData, firstName, templateName) => {
    ...
    <br/> your template named ${templateName} `

    if (statusData && statusData.firstLocalizationStatus) {
      html += 'has been updated to - ' + statusData.firstLocalizationStatus + '<br/>'
    }
    if (statusData && statusData.secondLanguageRequired) {
      html += ' second translation has been updated to - ' + statusData.secondLocalizationStatus + '<br/>'
    }
    if (statusData && statusData.firstLocalizationRejectionReason) {
      html += ' Rejected reason - ' + statusData.firstLocalizationRejectionReason + '<br/>'
    }
    if (statusData && statusData.secondLanguageRequired &&
            statusData.secondLocalizationRejectionReason) {
      html += ' Rejected reason - ' + statusData.secondLocalizationRejectionReason + '.'
    }
