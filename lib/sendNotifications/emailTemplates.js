...
<tr><![endif]--><!--[if mso]><td valign="top" width="600" style="width:600px;"><![endif]--><table align="left" border="0" cellpadding="0" cellspacing="0" style="max-width:100%;min-width:100%;border-collapse:collapse;mso-table-lspace:0;mso-table-rspace:0;-ms-text-size-adjust:100%;-webkit-text-size-adjust:100%" width="100%" class="mcnTextContentContainer"><tbody><tr><td valign="top" class="mcnTextContent" style="padding-top:0;padding-right:18px;padding-bottom:9px;padding-left:18px;mso-line-height-rule:exactly;-ms-text-size-adjust:100%;-webkit-text-size-adjust:100%;word-break:break-word;color:#fff;font-family:Helvetica;font-size:12px;line-height:150%;text-align:center">Copyright © 2021 Helo Whatsapp. All Rights reserved.</td></tr></tbody></table><!--[if mso]></td><![endif]--><!--[if mso]></tr>
  ...
  <body>
    ...
    <tr style="background-color:red;color:white;font-weight:bold">
        ...
        <th>Business Initiated</th>
        <th>User Initiated</th>
        <th style="background-color:blue;color:white;font-weight:bold">Total Message</th>
        <th style="background-color:blue;color:white;font-weight:bold">Outgoing Message</th>
        <th style="background-color:blue;color:white;font-weight:bold">Incoming Message</th>
    </tr>
    ${billingStatusData.map((q) => {
        totalLastdayUiCount += convoLastday[q[0]]?.ui || 0
        totalLastdayBiCount += convoLastday[q[0]]?.bi || 0
        totalLastdayOutgoingCount += outgoingLastday[q[0]]?.totalOutgoing || 0
            return `<tr>
        <td>${userIdToUserNameBilling[q[0]] || q[0]}</td>
        <td>${convoLastday[q[0]]?.ui + convoLastday[q[0]]?.bi || 0}</td>
        <td>${convoLastday[q[0]]?.bi || 0}</td>
        <td>${convoLastday[q[0]]?.ui || 0}</td>
        <td>${q[1][0]  + outgoingLastday[q[0]]?.totalOutgoing || 0}</td>
        <td>${outgoingLastday[q[0]]?.totalOutgoing || 0}</td>
        <td>${q[1][0] || 0}</td>
    </tr>`
        }).join('')}
    <tr style="background-color:#0000001c;">
        <td><b>Total</b></td>
        <td><b>${totalLastdayUiCount + totalLastdayBiCount}</b></td>
        <td><b>${totalLastdayBiCount}</b></td>
        <td><b>${totalLastdayUiCount}</b></td>
        <td><b>${totalBillingMessageCount + totalLastdayOutgoingCount}</b></td>
        <td><b>${totalLastdayOutgoingCount}</b></td>
        <td><b>${totalBillingMessageCount}</b></td>
    </tr>
  </table>
    <br />
    <br />
