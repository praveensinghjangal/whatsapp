...
_.each(swaggerInternalDocument.paths, (pathObj, pathName) => {
  _.each(pathObj, async (methodObj, methodName) => {
    let hostName = __config.swaggerUrl[methodObj['x-module']]
      ? __config.swaggerUrl[methodObj['x-module']]
      : __config.swaggerUrl.platform
    hostName = hostName.split('://')[1]
    if (methodObj && methodObj['x-isPublic'] === true) {
      pubTagNameArr.push(methodObj.tags)
      if (swaggerPublicDocument.paths[hostName + pathName]) {
    ...
    if (swaggerPublicDocument.paths[pathName]) {
      ...
      swaggerInternalDocument.paths[hostName + pathName] = {
        ...swaggerInternalDocument.paths[hostName + pathName],
...
var swaggerOptions = {
  customCss: '.swagger-ui .topbar-wrapper img {content:url("https://helo.co.in/images/logo.png");} .swagger-ui section.models {display: none} ' +
    ...
    .swagger-ui #operations-message-send.opblock .opblock-summary-description
    {font-size:13px;
      -webkit-box-flex:1;
      -ms-flex:1;flex:1;
    ...
    .swagger-ui #operations-message-sendBulk.opblock .opblock-summary-description
    {font-size:13px;
      -webkit-box-flex:1;
      -ms-flex:1;flex:1;
      font-family:Open Sans,sans-serif;
    ...
    .swagger-ui #operations-message-sendSingle.opblock .opblock-summary-description
    {font-size:13px;
      -webkit-box-flex:1;
      -ms-flex:1;flex:1;
      font-family:Open Sans,sans-serif;
...
module.exports = (app, routeUrlPrefix) => {
  ...
  app.use(
    routeUrlPrefix + '/internal-docs/:server_doc_api_key/*',
    function (request, response, next) {
      ...
      if (host.indexOf(__config.port) <= -1) {
        __logger.info('Unauthorized request for server doc', {
          m: __m,
          req_id: request.id,
          ip_address: request.req_ip,
          host: host
        })
        return response.send('Unauthorized')
      }
      __logger.info('got request for server doc', {
        m: __m,
        req_id: request.id,
        ip_address: request.req_ip,
        host: host
      ...
      } else if (apiKey === __config.authConfig.serverDocAccessKey) {
        next()
      } else {
        response.send('Unauthorized')
      }
