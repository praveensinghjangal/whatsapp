 ...
 * UPDATED BY : DANISH.GALIYARA [23/9/2020]
 */
...
_.each(swaggerInternalDocument.paths, (pathObj, pathName) => {
  // console.log('path--->', pathName, pathObj)
  _.each(pathObj, (methodObj, methodName) => {
    if (methodObj && methodObj.isPublic === true) {
      pubTagNameArr.push(methodObj.tags)
      if (swaggerPublicDocument.paths[pathName]) {
        swaggerPublicDocument.paths[pathName][methodName] = methodObj
      } else {
        swaggerPublicDocument.paths[pathName] = { [methodName]: methodObj }
      }
    }
...
pubTagNameArr = [...new Set(pubTagNameArr.flat(1))]
