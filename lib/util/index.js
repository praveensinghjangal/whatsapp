...
function send (response, options) {
  //__logger.info(response, options);
...
function checkRequiredMissingParam (request, requiredParams) {
    ...
    if (isArray(request)) {
      if (request.length > 0) {
        for (var i = 0; i < request.length; i++) {
          ...
          if (missingKeys) {
           __logger.info('Missing parameter [' + missingKeys + '] in array of object =>' + JSON.stringify(request[i]))
  ...
  if (_.isEmpty(missingRequiredParamMsg)) { return false } else { return missingRequiredParamMsg }
}
utils.checkRequiredMissingParam = checkRequiredMissingParam

function getTrueFalseValue (input) {
  if (input == 1 || input == '1' || input == true || input == 'true') { return true } else { return false }
}
...
function getHmac (text, key, encodeType) {
  ...
  if (encodeType) { encodeMethod = encodeType }
...
function getValidUpdateObject (data) {
  ...
  if (!isEmpty(data) && !isArray(data) && typeof data === 'object') {
    for (var key in data) {
      var value = data[key]
      if (value !== __constants.CUSTOM_CONSTANT.NOT_TO_UPDATE) {
...
function generate6CharShortCode (text) {
  ...
  if (text.length < 10 || reverse_code.indexOf('0') == 0 || reverse_code == text) { return null }
  ...
  if (object_json) {
    for (var key in object_json) {
      if (typeof object_json[key] === 'string' ||
                ...
                typeof object_json[key] === 'boolean') { text = text + key + ':' + object_json[key] + ';' }
    }
  }
  return text
}
...
function checkValidDevicePlatform (devicePlatform) {
  var dp = null
  for (var key in __constants.CUSTOM_CONSTANT.DEVICE_PLATFORM) {
    if (devicePlatform.toLowerCase() == __constants.CUSTOM_CONSTANT.DEVICE_PLATFORM[key].toLowerCase()) {
      dp = __constants.CUSTOM_CONSTANT.DEVICE_PLATFORM[key]
    }
  }
  return dp
}
...
function getFromAndToDate (fromDate, toDate, format, onlyDays) {
  try {
    if (typeof fromDate === 'string' || typeof fromDate === 'number') { fromDate = new Date(fromDate) }
...
function getBrowserName (browser) {
  if (browser) {
    ...
    for (var key in browser) {
      if (browser[key] == true) { nameList.push(key) }
    }
...
function getMessageLength (message) {
  ...
  try {
    //__logger.info('message length by .length() function', {m: __m, message_length: message.length});
    if (Buffer.isBuffer(message)) {
      ...
      for (var i = 0; i < message.length; i++) {
        ...
        var char_count = 1
        switch (true) {
          case ((ascii_value >= 32 && ascii_value <= 90) || (ascii_value >= 97 && ascii_value <= 122) || __constants.CUSTOM_CONSTANT.CHAR_COUNT.ONE.includes(ascii_value)):
            char_count = 1
            break
          case (__constants.CUSTOM_CONSTANT.CHAR_COUNT.TWO.includes(ascii_value)):
            char_count = 2
            break
          case (__constants.CUSTOM_CONSTANT.CHAR_COUNT.THREE.includes(ascii_value)):
            char_count = 3
            break
        }
    ...
    //__logger.info('message length by custom logic', {m: __m, message_length: message_length});
  } catch (e) {
...
function getMessageCreditCount (message, unicode) {
  ...
  if (_.isNumber(msg_credits)) { return { err_msg: null, msg_credit_count: msg_credits } }
...
function credit_calculation (sms_body) {
  ...
  var err_msg = null
  if (sms_body.message) { default_message_length = getMessageLength(sms_body.message) }
  ...
  for (var i = 0; i < sms_body.list.length; i++) {
    ...
    if (_.isNumber(msg_credits)) { total_credits += msg_credits } else {
      err_msg = msg_credits
      break
    }
  }
...
function validateSmsRequest (sms_body) {
  ...
  if (sms_body.list.length > __constants.CUSTOM_CONSTANT.MAX_BULK_SMS_REQUEST_SIZE) {
    sms_body.err_msg = 'maximum number list size[' + __constants.CUSTOM_CONSTANT.MAX_BULK_SMS_REQUEST_SIZE + '] exceeded'
    return sms_body
  }
...
function validateSmsRequestWithCallback (sms_body, _callback) {
  ...
  async.each(sms_body.list, function (mobile, fcallback) {
    ...
    if (list_type == 'object') {
      ...
      if (sms_body.err_msg) { return fcallback(sms_body.err_msg) }
...
function createDirectoryPath (basePath, foldersNestedList, callback) {
  ...
  async.eachSeries(foldersNestedList, function (dirName, cb) {
    ...
    if (err) {
     ...
     __logger.info('created file upload directory:', filePath)
...
function extractHeloUrlPattern (camp_id, message) {
  // <http://h1o.in/xxxx>
  var is_url = false; var pattern = null; var err = true; var domain = ''
