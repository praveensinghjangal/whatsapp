...
module.exports = (entityData, entityName, entityRowId, userId) => {
  ...
  __logger.info('Inside saveDataHistory')
  ...
  validateInput({ entityData, entityName, entityRowId, userId })
    .then(validData => __db.mysql.query(__constants.HW_MYSQL_NAME, query, [uniqueId.uuid(), JSON.stringify(entityData), entityName, entityRowId, userId]))
    .then(result => {
