...
class rabbitmqLib {
  ...
  sendToQueue (queue, message) {
    return new Promise((resolve, reject) => {
      __logger.debug('rabbitmqLib.sendToQueue, got request', { q_name: queue.q_name, message: Humanize.truncate(message, __config.rabbitmq_vb.truncate_message_log_length) })
            if (this.connection) {
        if (this.channel[queue.q_name]) {
          this.channel[queue.q_name].sendToQueue(queue.q_name, new Buffer(message), { persistent: true })
                    __logger.debug('rabbitmqLib.sendToQueue, queued success', { q_name: queue.q_name, message: Humanize.truncate(message, __config.rabbitmq_vb.truncate_message_log_length) })
                    resolve(null)
                } else {
  ...
  publishToExchange (exchange, r_key, message) {
    return new Promise((resolve, reject) => {
      var vm = this
            __logger.debug('rabbitmqLib.publishToExchange, got request', { q_name: exchange.q_name, r_key: r_key, message: Humanize.truncate(message, __config.rabbitmq_vb.truncate_message_log_length) })
            if (vm.connection) {
        if (vm.channel[exchange.q_name]) {
          vm.channel[exchange.q_name].publish(exchange.q_name, r_key, new Buffer(message), { persistent: true })
                    __logger.debug('rabbitmqLib.publishToExchange, queued success', { q_name: exchange.q_name, r_key: r_key, message: Humanize.truncate(message, __config.rabbitmq_vb.truncate_message_log_length) })
                    resolve(null)
                } else {
