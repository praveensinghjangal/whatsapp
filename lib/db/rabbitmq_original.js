...
class rabbitmqLib {
  constructor () {
   __logger.info('rabbitmqLib.constructor called.')
  ...
  handleError (err) {
    __logger.error('rabbitmqLib.handleError, error in connection', { amqp_url: __config.rabbitmq_vb.amqp_url, err: err })
        console.error(err)
        this.connection = null
        process.exit(1)
        ...
        //         __logger.warn('rabbitmqLib.handleError, reconnecting', { amqp_url: __config.rabbitmq_vb.amqp_url });
        //     }
        // }, __config.rabbitmq_vb.reconnect_interval);
    };

  createExchange (exchange_name, exchange_type, options) {
    if (this.connection) {
      this.connection.createChannel().then((ch) => {
       __logger.info('rabbitmqLib.createExchange, declaring a exchange', { exchange_name: exchange_name })
                ch.on('error', (err) => {
  ...
  createBind (q_name, exchange_name, routing_key) {
    if (this.connection) {
      this.connection.createChannel().then((ch) => {
       __logger.info('rabbitmqLib.createBind, declaring a bind', { q_name: q_name, exchange_name: exchange_name })
      ...
      if (!vm.channel[queue.q_name]) {
        await vm.connection.createChannel().then((ch) => {
          vm.channel[queue.q_name] = ch
                   __logger.info('lib_amqp.createChannelsForQueue, declaring a queue', { q_name: queue.q_name })
  ...
  sendToQueue (queue, message) {
    return new Promise((resolve, reject) => {
     __logger.info('rabbitmqLib.sendToQueue, got request', { q_name: queue.q_name, message: Humanize.truncate(message, __config.rabbitmq_vb.truncate_message_log_length) })
            if (this.connection) {
        if (this.channel[queue.q_name]) {
          this.channel[queue.q_name].sendToQueue(queue.q_name, new Buffer(message), { persistent: true })
                   __logger.info('rabbitmqLib.sendToQueue, queued success', { q_name: queue.q_name, message: Humanize.truncate(message, __config.rabbitmq_vb.truncate_message_log_length) })
                    resolve(null)
                } else {
          this.connection.createChannel().then(function (ch) {
            this.channel[queue.q_name] = ch
                        ...
                        ok.then((_qok) => {
             __logger.info('rabbitmqLib.sendToQueue, queueDeclared', { q_name: queue.q_name })
                            this.channel[queue.q_name].sendToQueue(queue.q_name, new Buffer(message), { persistent: true })
                           __logger.info('rabbitmqLib.sendToQueue, queued success', { q_name: queue.q_name, message: Humanize.truncate(message, __config.rabbitmq_vb.truncate_message_log_length) })
                            resolve(null)
                        })
  ...
  publishToExchange (exchange, r_key, message) {
    return new Promise((resolve, reject) => {
      var vm = this
           __logger.info('rabbitmqLib.publishToExchange, got request', { q_name: exchange.q_name, r_key: r_key, message: Humanize.truncate(message, __config.rabbitmq_vb.truncate_message_log_length) })
            if (vm.connection) {
        if (vm.channel[exchange.q_name]) {
          vm.channel[exchange.q_name].publish(exchange.q_name, r_key, new Buffer(message), { persistent: true })
                   __logger.info('rabbitmqLib.publishToExchange, queued success', { q_name: exchange.q_name, r_key: r_key, message: Humanize.truncate(message, __config.rabbitmq_vb.truncate_message_log_length) })
                    resolve(null)
                } else {
          vm.connection.createChannel().then((ch) => {
            vm.channel[exchange.q_name] = ch
                        ...
                        ok.then((_qok) => {
             __logger.info('rabbitmqLib.publishToExchange, queueDeclared', { q_name: exchange.q_name })
                            vm.channel[exchange.q_name].publish(exchange.q_name, r_key, new Buffer(message), { persistent: true })
                           __logger.info('rabbitmqLib.publishToExchange, queued success', { q_name: exchange.q_name, r_key: r_key, message: Humanize.truncate(message, __config.rabbitmq_vb.truncate_message_log_length) })
                            resolve(null)
                        })
  ...
  //         var vm = this;
  //         if (vm.connection) {
  //            __logger.info('rabbitmqLib.fetchFromQueue, waiting for message');
  //             vm.channel[queue].consume(queue, (msg) => {
  //                __logger.info('rabbitmqLib.fetchFromQueue, got message', { r_key: (msg.fields) ? msg.fields.routingKey : "", message: Humanize.truncate(msg.content.toString(), __config.rabbitmq_vb.truncate_message_log_length) });
  //                 return Promise.resolve({
  //                     msg: msg,
  //                     ack: (ack_msg) => {
  //                         if (ack_msg) {
  //                             vm.channel[queue.q_name].ack(msg);
  //                            __logger.info('rabbitmqLib.fetchFromQueue, ack message', { r_key: (msg.fields) ? msg.fields.routingKey : "", message: Humanize.truncate(msg.content.toString(), __config.rabbitmq_vb.truncate_message_log_length) });
  //                         }
  //                         else {
  //                             vm.channel[queue.q_name].nack(msg);
  //                            __logger.info('rabbitmqLib.fetchFromQueue, nack message', { r_key: (msg.fields) ? msg.fields.routingKey : "", message: Humanize.truncate(msg.content.toString(), __config.rabbitmq_vb.truncate_message_log_length) });
  //                         }
  //                     }
  //                 });
  //             }, { noAck: true });
