...
lib_postgresql.__getClient = function (__cb) {
   ...
   __logger.info('got request for postgresql client connection', {m: __m, host: __config.postgresql.options.host});
...
lib_postgresql.__query = function (pgQuery, queryParam, __cb) {
   ...
   __logger.info('got request for postgresql query', {m: __m, host: __config.postgresql.options.host});
    if (lib_postgresql.conn) {
        lib_postgresql.conn.query(pgQuery, queryParam, function (err, result) {
            if (err) {
               ...
               __logger.info('Query success:', {m: __m, host: __config.postgresql.options.host, query: pgQuery, queryParam: queryParam});
                __cb(null, result);
            }
        });
    }
