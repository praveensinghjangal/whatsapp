...
lib_es.__callAPI = function (apiType, esQuery, __cb) {
   ...
   __logger.info('request for elasticsearch API "' + apiType+'"', {m: __m, host: __config.elasticsearch.options.host});
    if (lib_es.conn) {
        try {
            lib_es.conn[apiType](esQuery, function (error, response) {
                if (error) {
                   ...
                   __logger.info('Query Success:', {m: __m, host: __config.elasticsearch.options.host, query: esQuery});
                    __cb(null, response);
                }
            });
        }
...
lib_es.__count = function (esQuery, __cb) {
    var __m = 'lib_es.__count';
   __logger.info('request for elasticsearch count', {m: __m, host: __config.elasticsearch.options.host});
    if (lib_es.conn) {
...
lib_es.__search = function (esQuery, __cb) {
   ...
   __logger.info('request for elasticsearch search', {m: __m, host: __config.elasticsearch.options.host});
    if (lib_es.conn) {
