...
class mongoLib {
    ...
    __bulkinsertWithBulkUpdate(db_name, collection_name, data) {
        return new Promise((resolve, reject) => {
            let vm = this;
            vm.__getMongoDbConnection(db_name).then((connection) => {
                let bulk = connection.collection(collection_name).initializeUnorderedBulkOp();
                if (data && data.length > 0) {
                    data.forEach(element => {
                        bulk.find({
                            "summaryDate": element['summaryDate'],
                            "wabaPhoneNumber": element['wabaPhoneNumber'],
    ...
    __campaignBulkInsert(db_name, collection_name, data) {
        return new Promise((resolve, reject) => {
            let vm = this;
            vm.__getMongoDbConnection(db_name).then((connection) => {
                let bulk = connection.collection(collection_name).initializeUnorderedBulkOp();
    ...
    __findSortField(db_name, collection_name, find_params, select_params, sort_params) {
        let mongoConfig = require('../../config/keysToEncrypt.json')
        __logger.debug('lib_mongo.__find, request', {
            db: db_name,
            collection: collection_name,
        ...
        return new Promise((resolve, reject) => {
            ...
            vm.__getMongoDbConnection(db_name).then((connection) => {
                select_params = select_params || {};
                sort_params = sort_params || {};
                connection.collection(collection_name).find(encrypyDecrypt.encryptKeysInObj(find_params, mongoConfig[collection_name] || [])).project(select_params).sort(sort_params).toArray((err, result) => {
                    if (err) {
                        __logger.error('lib_mongo.__find, failed', {
                            db: db_name,
                            collection: collection_name,
    ...
    __removeBulkFile(db_name, collection_name, data) {
        return new Promise((resolve, reject) => {
            let vm = this;
            vm.__getMongoDbConnection(db_name).then((connection) => {
                let bulk = connection.collection(collection_name).initializeUnorderedBulkOp();
                if (data && data.length > 0) {
                    data.forEach(element => {
                        ...
                        }).update({ $set: {isActive: '0', updateOn: new Date()}});
                    });
