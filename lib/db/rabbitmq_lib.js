...
class rabbitmqLib {
  ...
  sendToQueue(queue, message, priorityNumber = 0) {
    try {
      return new Promise((resolve, reject) => {
        __logger.info('rabbitmqLib.sendToQueue, got request', { q_name: queue.q_name, message: Humanize.truncate(message), priority: priorityNumber })
        let that = this
        if (this.connection) {
          if (this.channel[queue.q_name]) {
            ...
            __logger.info('rabbitmqLib.sendToQueue, queued success', { q_name: queue.q_name, message: Humanize.truncate(message) })
            resolve(null)
          } else {
            this.createChannelsForQueue(queue).then(function (ch) {
              __constants.MQ[queue.q_name] = JSON.parse(JSON.stringify(__constants.MQ['default']))
              __constants.MQ[queue.q_name].q_name = queue.q_name
              __logger.info('rabbitmqLib.sendToQueue, queueDeclared', { q_name: queue.q_name })
              that.sendToQueue(queue, message, priorityNumber = 0);
            })
            __logger.info('rabbitmqLib.sendToQueue, queued success', { q_name: queue.q_name, message: Humanize.truncate(message) })
            resolve(null)
          }
      ...
      console.log('catch in sendToQueue main function', e)
    }
  }
  ...
  publishToExchange(exchange, r_key, message) {
    return new Promise((resolve, reject) => {
      var vm = this
      __logger.info('rabbitmqLib.publishToExchange, got request', { q_name: exchange.q_name, r_key: r_key, message: Humanize.truncate(message) })
      if (vm.connection) {
        if (vm.channel[exchange.q_name]) {
          vm.channel[exchange.q_name].publish(exchange.q_name, r_key, new Buffer(message), { persistent: true })
          __logger.info('rabbitmqLib.publishToExchange, queued success', { q_name: exchange.q_name, r_key: r_key, message: Humanize.truncate(message) })
          resolve(null)
        } else {
