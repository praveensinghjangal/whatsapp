...
class MysqlLib {
          ...
          vm.connections[connName].conn.getConnection((err, connection) => {
            if (err) {
              ...
              const sql = connection.query(queryString, queryParam, (err, result) => {
                ...
                if (err) {
                  console.log('query firing error :-', err)
  ...
  batch(connName, queryString, queryParam) {
    return new Promise((resolve, reject) => {
      var vm = Object.assign({}, this)
      __logger.info('maraiaLib.query request::', { conn_name: connName, host: (config[connName] && config[connName].options && config[connName].options.host), query: queryString.substr(0, 50) })
      if (config[connName] && config[connName].init) {
        if (vm.connections[connName] && vm.connections[connName].conn) {
          vm.connections[connName].conn.getConnection()
            .then(connection => {
              if (config[connName].is_slave && !vm.isSelectQuery(queryString)) {
                __logger.error('maraiaLib.query slave support only select query:', { conn_name: connName, host: config[connName].options.host, query: queryString.substr(0, 30) })
                return reject(new Error('slave support only select query'))
              }
              const sql = connection.batch(queryString, queryParam)
                .then(result => {
                  __logger.info('maraiaLib.query success ::', { conn_name: connName, host: (config[connName].options && config[connName].options.host) })
                  connection.release()
                  return resolve(result)
                })
                .catch(err => {
                  __logger.error('maraiaLib.query error in query ::', { err_code: err.code, err_msg: err.sqlMessage, conn_name: connName, host: (config[connName].options && config[connName].options.host), query: sql.sql || '' })
                  return reject(err)
                })
            })
            .catch(err => {
              __logger.error('maraiaLib.query connection failed with mariadb', { conn_name: connName, host: config[connName].options.host, err: err })
              return reject(new Error('mariadb connection failed'))
            })
        } else {
          __logger.error('maraiaLib.query mariadb[' + connName + '] not initialized.')
          return reject(new Error('mariadb[' + connName + '] connection not initialized'))
        }
      } else {
        __logger.error('maraiaLib.query mariadb[' + connName + '] connection is not define in config.')
        return reject(new Error('mariadb[' + connName + '] connection not define'))
      }
    })
  }
}
