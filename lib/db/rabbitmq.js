...
class rabbitmqLib {
  ...
  sendToQueue (queue, message) {
    return new Promise((resolve, reject) => {
     __logger.info('rabbitmqLib.sendToQueue, got request', { q_name: queue.q_name, message: Humanize.truncate(message, __config.rabbitmq.truncate_message_log_length) })
      if (this.connection) {
        if (this.channel[queue.q_name]) {
          this.channel[queue.q_name].sendToQueue(queue.q_name, new Buffer(message), { persistent: true })
         __logger.info('rabbitmqLib.sendToQueue, queued success', { q_name: queue.q_name, message: Humanize.truncate(message, __config.rabbitmq.truncate_message_log_length) })
          resolve(null)
        } else {
  ...
  publishToExchange (exchange, r_key, message) {
    return new Promise((resolve, reject) => {
      var vm = this
     __logger.info('rabbitmqLib.publishToExchange, got request', { q_name: exchange.q_name, r_key: r_key, message: Humanize.truncate(message, __config.rabbitmq.truncate_message_log_length) })
      if (vm.connection) {
        if (vm.channel[exchange.q_name]) {
          vm.channel[exchange.q_name].publish(exchange.q_name, r_key, new Buffer(message), { persistent: true })
         __logger.info('rabbitmqLib.publishToExchange, queued success', { q_name: exchange.q_name, r_key: r_key, message: Humanize.truncate(message, __config.rabbitmq.truncate_message_log_length) })
          resolve(null)
        } else {
  ...
  //                __logger.info('rabbitmqLib.fetchFromQueue, got message', { r_key: (msg.fields) ? msg.fields.routingKey : "", message: Humanize.truncate(msg.content.toString(), __config.rabbitmq.truncate_message_log_length) });
  //                 return Promise.resolve({
  //                     msg: msg,
  //                     ack: (ack_msg) => {
  //                         if (ack_msg) {
  //                             vm.channel[queue.q_name].ack(msg);
  //                            __logger.info('rabbitmqLib.fetchFromQueue, ack message', { r_key: (msg.fields) ? msg.fields.routingKey : "", message: Humanize.truncate(msg.content.toString(), __config.rabbitmq.truncate_message_log_length) });
  //                         }
  //                         else {
  //                             vm.channel[queue.q_name].nack(msg);
  //                            __logger.info('rabbitmqLib.fetchFromQueue, nack message', { r_key: (msg.fields) ? msg.fields.routingKey : "", message: Humanize.truncate(msg.content.toString(), __config.rabbitmq.truncate_message_log_length) });
  //                         }
  //                     }
  //                 });
  //             }, { noAck: true });
