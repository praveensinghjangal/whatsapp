...
const errorToTelegram = (error, message) => {
    ...
    http.Get(__constants.TELEGRAM_API + __config.botToken + __constants.TELEGRAM_ENDPOINTS.sendMessage + '?chat_id=' + __config.telegramChatId + '&text=' + error + '\n' + message)
      .then(data => {
        data = data.body || data
  ...
  const telegramMessage = (message) => {
    ...
    http.Get(__constants.TELEGRAM_API + __config.supportBotToken + __constants.TELEGRAM_ENDPOINTS.sendMessage + '?chat_id=' + __config.supportTelegramChatId + '&text=' + message )
      .then(data => {       
        data = data.body || data
        __logger.info('response of telegram >>>>>>>>>>>>>>>>>>>>>>>>..', data)
        if (data && data.code && data.code === 2000) {
  ...
  module.exports = { send: errorToTelegram, sendTelegramMessage:telegramMessage}
