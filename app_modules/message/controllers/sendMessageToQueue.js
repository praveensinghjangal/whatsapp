...
const sendToQueue = (data, providerId, userId, maxTpsToProvider, authToken) => {
...
const sendToQueueBulk = (data, providerId, userId, maxTpsToProvider, authToken) => {
  let p = q()
  const thePromises = []
  data.forEach(singleObject => {
    p = p.then(() => sendToQueue(singleObject, providerId, userId, maxTpsToProvider, authToken))
      .catch(err => err)
    thePromises.push(p)
  })
...
const singleRuleCheck = (data, wabaPhoneNumber, index) => {
  ...
  if (data && data.whatsapp) {
    ...
    checkIfNoExists(data.whatsapp.from)
      ...
      .then(noValRes => {
        data.redisData = noValRes.data.redisData || {}
        return templateParamValidationService.checkIfParamsEqual(data.whatsapp.template, data.whatsapp.from)
      })
  ...
  validate.sendMessageToQueue(req.body)
    ...
    .then(isValid => {
      ...
      if (invalidReq.length > 0) {
        ...
        return sendToQueueBulk(req.body, req.user.providerId, req.user.user_id, req.user.maxTpsToProvider, req.headers.authorization)
      }
    })
    .then(sendToQueueRes => {
