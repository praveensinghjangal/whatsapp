...
senderid_func.create_senderid = async (req, res) => {
    __util.make_log_statement(req, "create_senderid");
    ...
    if (!req.body.sample_content) {
        return __util.send(res, {
            type: __define.RESPONSE_MESSAGES.INVALID_REQUEST,
            data: { message: "sample_content not provided" }
        });
    };
    var access = false
    var user_id = req.query.user_id;
    var checkuser_sql_query = "select package_selected from helootplocal.users where user_id like?";
    ...
    try {
       if(result.length != 0 && result[0].package_selected != 'Demo') {
           access = true
       }
    } catch (e) {
        res.send(e)
    ...
    if (access) {
        const sender_uuid = uuid4();
        const sender_id = req.body.sender_id;
        const sample_content = req.body.sample_content;
        const approval_status = 'pending';
        const status = 'active';
        var created_at = new Date();
        var modified_at = new Date();

        __db.mysql.query("mysql_user_local_db", "insert into senderid (user_id, sender_uuid, sender_id, sample_content, approval_status, status,  created_at, modified_at) values (?,?,?,?,?,?,?,?)", [user_id, sender_uuid, sender_id, sample_content, approval_status, status, created_at, modified_at]).then((results) => {
            // console.log(results)
            __util.send(res, {
                type: __define.RESPONSE_MESSAGES.SUCCESS,
                data: { message: "sendeid created successfully", "sender_uuid": sender_uuid }
            })
        }).catch(err => {
            console.log(err)
            if (err.code === "ER_DUP_ENTRY") {
                return __util.send(res, {
                    type: __define.RESPONSE_MESSAGES.INVALID_REQUEST,
                    data: { message: "sender_id already exists" }
                });
            } else {
                __logger.error("error: ", err);
                return __util.send(res, {
                    type: __define.RESPONSE_MESSAGES.INVALID_REQUEST,
                    data: { message: "Problem while creating senderid" }
                });
            }
        });
    } else {
        ...
        return __util.send(res, {
            ...
            data: { message: "not allowed to run this task" }
        });
    }
};

senderid_func.view_senderid = (req, res) => {
    __util.make_log_statement(req, "view_senderid");
    if (!req.query.user_id) {
        return __util.send(res, {
            type: __define.RESPONSE_MESSAGES.INVALID_REQUEST,
            data: { message: "user_id not provided" }
    ...
    var user_id = req.query.user_id;
    var sql_query = "select sender_uuid, sender_id, sample_content, approval_status, status, created_at from helootplocal.senderid where user_id like ?";
    var sql_param = [user_id];

    __db.mysql.query("mysql_user_local_db", sql_query, sql_param).then((results) => {
        var senderid_count = results.length;
                res.send({
                    code: 2000,
                    status: true,
                    data: {
                        recordsFiltered: senderid_count,
                        recordsTotal: senderid_count,
                        records: results,
                    }
                })
...
senderid_func.view_single_senderid = (req, res) => {
    __util.make_log_statement(req, "view_single_senderid");
    if (!req.query.user_id) {
        return __util.send(res, {
            type: __define.RESPONSE_MESSAGES.INVALID_REQUEST,
            data: { message: "user_id not provided" }
    ...
    var user_id = req.query.user_id;
    var sender_uuid = req.params.sender_uuid;
    if (user_id) {
        var sql_query = "select sender_uuid, sender_id, sample_content, approval_status, status from helootplocal.senderid where sender_uuid like ? and user_id like ?";
        var sql_param = [sender_uuid,user_id];

        __db.mysql.query("mysql_user_local_db", sql_query, sql_param).then((results) => {
            if (results.length === 0) {
                __util.send(res, {
                    type: __define.RESPONSE_MESSAGES.NOT_FOUND,
                    data: { message: "senderid not found" }
                })
            } else {
                __util.send(res, {
                    type: __define.RESPONSE_MESSAGES.SUCCESS,
            ...
            __util.send(res, {
                type: __define.RESPONSE_MESSAGES.INVALID_REQUEST,
                data: { "message": "error in finding senderid" }
            });
        });
    } else {
        __util.send(res, {
...
senderid_func.approve_senderid = (req, res) => {
    if (!req.query.user_id) {
        return __util.send(res, {
            type: __define.RESPONSE_MESSAGES.INVALID_REQUEST,
            data: { message: "user_id not provided" }
    ...
    if(user_id == '515adee1-7f8e-48a8-9fcc-0cfe7016f1c8') {
        let sender_uuid = req.params.sender_uuid;
        let query_param = [approval_status,status,sender_uuid];
        
        __db.mysql.query("mysql_user_local_db", 'update senderid set approval_status = ?, status = ?  where sender_uuid=? ', query_param).then((results) => {
            // console.log(results)
            __util.send(res, {
                type: __define.RESPONSE_MESSAGES.SUCCESS,
                data: { message: "senderid approved successfully", "sender_uuid": sender_uuid }
            })
        }).catch(err => {
            __logger.error("error: ", err);
            return __util.send(res, {
...
senderid_func.change_status_senderid = async (req, res) => {
    __util.make_log_statement(req, "change_status_senderid");
    if (!req.query.user_id) {
        return __util.send(res, {
            type: __define.RESPONSE_MESSAGES.INVALID_REQUEST,
            data: { message: "user_id not provided" }
    ...
    let sender_uuid = req.params.sender_uuid;
    let mysql_check_query = "select * from senderid where sender_uuid = ? and user_id = ?";
    let mysql_check_params = [sender_uuid, user_id]
    let status = req.body.status;
    let query_param = [status,sender_uuid,user_id];
    let access = false;
   ...
   if(access) {
    __db.mysql.query("mysql_user_local_db", 'update senderid set status = ? where sender_uuid=? and user_id= ?', query_param).then((results) => {
        if(status === "active") {
            __util.send(res, {
                type: __define.RESPONSE_MESSAGES.SUCCESS,
        ...
        return __util.send(res, {
            type: __define.RESPONSE_MESSAGES.INVALID_REQUEST,
            data: { message: "Problem while updating senderid" }
        });
    ...
    __util.send(res, {
        ...
        data: { message: "not such user/senderid" }
    })
   }
};
