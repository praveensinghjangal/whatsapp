  ...
  startServer () {
    ...
    __db.init().then(result => {
      ...
      rmq_object.channel[queue].consume(queue, (mq_message) => {
        ...
        const sql_query = 'select cm.destination, cm.circle, cm.operator, \n' +
                    ...
                    'cm.billsec, ' + __constants.MYSQL_QUERY.cdr_reason + ", cm.hangup_cause, if(cm.reason='SUCCESS',dp.dtmf,'') as dtmf \n" +
